...
exports.handler = (event, context, callback) => {
    ...
    connection.connect(function(err) {
        ...
        var queryString = 'SELECT tin.*, blend.name, blend.maker, blend.country, blend.cut, blend.style, '+
            'blend.image_file, blend.blending_component FROM tin, blend WHERE tin.blend=blend.id ORDER by acquire_date, tin_number;';
        if (event.blendID) {
            queryString = 'SELECT tin.*, blend.name, blend.maker, blend.country, blend.cut, blend.style, '+
                        ...
                        'blend.id='+connection.escape(event.blendID)+' ORDER by acquire_date, tin_number;';
        } 
